  ...
  return (
    <div className="relative">
      ...
      <Button
        variant="secondary"
        size="icon"
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 rounded-full shadow-md h-8 w-8"
      ...
      <Button
        variant="secondary"
        size="icon"
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 rounded-full shadow-md h-8 w-8"
        onClick={() => scrollToPromotion(currentIndex + 1)}
      >
      ...
      <div className="flex justify-center gap-1 mt-2">
        {promotions.map((_, index) => (
          <button
            key={index}
            className={`h-2 rounded-full transition-all ${currentIndex === index ? "w-4 bg-primary" : "w-2 bg-muted"}`}
