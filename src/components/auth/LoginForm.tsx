    ...
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          ...
          queryParams: {
            // Solicitar acceso offline para obtener refresh token
            access_type: "offline",
            // Forzar el consentimiento para asegurar que siempre se obtenga un refresh token
            prompt: "consent",
          },
          // Asegurarse de que el navegador redirija automáticamente
      ...
      // Si llegamos aquí y tenemos una URL, redirigir manualmente
      if (data?.url) {
        console.log("Redirigiendo a:", data.url);
        window.location.href = data.url;
      } else {
        console.log("OAuth iniciado exitosamente, esperando redirección automática...");
      }
    } catch (err: any) {
